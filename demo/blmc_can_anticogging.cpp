...
#endif
{
    int ret, count = 0, print = 12000;
    CAN_Frame_t frame;

    float pos = 0;
    float current = 0.;
    float calib_step = 1./180.;
    float calib_pos_threshold = 0.1*calib_step;
    float calib_vel_threshold = 1e-1;

    bool enabled = false;

    std::ifstream anticogging_conf ("anticogging.conf");
    std::map<int,float> anticogging;

    float file_pos, file_offset;
    while (anticogging_conf >> file_pos >> file_offset) {
      // rt_printf("%0.3f, %0.3f\n", file_pos, file_offset);
      anticogging[(int)(file_pos)] = file_offset;   
    }
    ...
    while (1) {
        ...
        if (!enabled && board_data.status.status.motor1_ready) {
            ...
            if (board_data.encoder_index[0].value == 0.) {
                // Rotate the motor over the encoder.
                BLMC_sendMotorCurrent(can_handle, dir *0.5, 0.); 
            } else {
                enabled = true;
        ...
        if (enabled) {
            ...
            if (dir * err < calib_pos_threshold) 
            {
                pos = pos_norm + dir * calib_step;

                current = 0.;
                rt_printf("Reset.\n");
            }

            if (dir * current < 0.5 && board_data.sync.velocity.value[0] == 0) {
                // Ramping this one up slowly.
                current += dir * 0.0001;
            }


            if (dir * current < 0. && dir * board_data.sync.velocity.value[0] < 0.03) {
                current = 0.;
                rt_printf("Wrong dir reset.\n");
            }

            // Prevent too fast motions.
            if (dir * board_data.sync.velocity.value[0] > 0.08) {
                if (dir * current > 0.2 || dir * current < 0.) {
                    rt_printf("Going too fast -> invert.\n");
                    current = dir * -0.2;
                } else {
                    rt_printf("Going too fast -> reset.\n");
                    current = 0.;
               }
            }
            ...
            float applied_current = current;
            int pos_idx = (int)((pos_norm + 0.001*board_data.sync.velocity.value[0]) * 5000.);
            if (pos_idx < 0) {
                pos_idx += 5000;
            }
            if (anticogging.find(pos_idx) != anticogging.end()) {
                if (anticogging[pos_idx] > 0.1 && board_data.sync.velocity.value[0] > 0){
                    applied_current += anticogging[pos_idx];
                } else if (anticogging[pos_idx] < -0.1 && board_data.sync.velocity.value[0] < 0){
                    applied_current += anticogging[pos_idx];
                }
            }

            BLMC_sendMotorCurrent(can_handle, applied_current, 0.);
            // rt_printf("Current=%0.3f, des=%0.5f\n", current, pos);

            // if (count % 10 == 0) {
            if (true) {
                fprintf(log_file, "0,%0.6f,%0.5f,%0.5f,%0.5f,%0.5f,%0.6f\n", 
                        pos_norm, 
